...
class DiameterErrorMsg {
 ...
 public:
	static std::auto_ptr < DiameterMsg > Generate(DiameterMsg & request,
		...
		DiameterIdentityAvpWidget originHostAvp((char *)
							DIAMETER_AVPNAME_ORIGINHOST);
		DiameterIdentityAvpWidget originRealmAvp((char *)
							 DIAMETER_AVPNAME_ORIGINREALM);
		DiameterUInt32AvpWidget resultCodeAvp((char *)
						      DIAMETER_AVPNAME_RESULTCODE);
		DiameterUInt32AvpWidget originStateAvp((char *)
						       DIAMETER_AVPNAME_ORIGINSTATEID);
		DiameterIdentityAvpWidget errorHostAvp((char *)
						       DIAMETER_AVPNAME_ERRORREPORTINGHOST);
		DiameterGroupedAvpWidget proxyInfoAvp((char *)
						      DIAMETER_AVPNAME_PROXYINFO);

		DiameterUtf8AvpContainerWidget sidAvp(errAnswer()->acl);
		diameter_utf8string_t *sidReq =
		    sidReqAvp.GetAvp((char *)DIAMETER_AVPNAME_SESSIONID);
		if (sidReq) {
			diameter_utf8string_t & sid =
			    sidAvp.AddAvp((char *)DIAMETER_AVPNAME_SESSIONID);
			sid = *sidReq;
		}

		diameter_unsigned32_t *oStateId =
		    originStateReqAvp.
		    GetAvp((char *)DIAMETER_AVPNAME_ORIGINSTATEID);
		if (oStateId) {
			originStateAvp.Get() = *oStateId;
			errAnswer()->acl.add(originStateAvp());
		}

		diameter_grouped_t *proxyInfo =
		    proxyInfoReqAvp.GetAvp((char *)DIAMETER_AVPNAME_PROXYINFO);
		if (proxyInfo) {
			DiameterGroupedAvpContainerWidget pInfoAvp(errAnswer()->
								   acl);
			diameter_grouped_t & pInfoAns =
			    pInfoAvp.AddAvp((char *)DIAMETER_AVPNAME_PROXYINFO);
			...
			diameter_identity_t *hostReq =
			    pHostReq.GetAvp((char *)DIAMETER_AVPNAME_PROXYHOST);
			diameter_identity_t & hostAns =
			    pHostAns.AddAvp((char *)DIAMETER_AVPNAME_PROXYHOST);
			hostAns = *hostReq;

			diameter_identity_t *stateReq =
			    pStateReq.
			    GetAvp((char *)DIAMETER_AVPNAME_PROXYSTATE);
			diameter_identity_t & stateAns =
			    pStateAns.
			    AddAvp((char *)DIAMETER_AVPNAME_PROXYSTATE);
			stateAns = *stateReq;
		}
