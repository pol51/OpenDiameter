...
EAPFAST_XMLElementParser::EAPFAST_XMLElementParser(std::string &tagName)
{
   this->name_ = tagName;
}

int EAPFAST_XMLElementParser::populate(DOMNode *n, DOMNode **found)
{
   DOMNode *sibling = n;
   while (sibling != NULL) {
...
char *EAPFAST_XMLElementParser::getTextContent(DOMNode *n)
{
...
int EAPFAST_XMLTreeParser::open(std::string &filename, EAPFAST_XMLElementParser &root)
{
   int rcode = (-1);
   ...
   try {
      ...
      EAP_LOG(LM_DEBUG, "[EAPFAST_XMLTreeParser::open]filename: %s\n",filename.c_str());
      try {
         parser.parse(filename.data());
         EAP_LOG(LM_DEBUG, "[EAPFAST_XMLTreeParser::open]ERROR No: %d\n",parser.getErrorCount());
...
void EAPFAST_XMLTreeParser::close()
{
   XMLPlatformUtils::Terminate();
}

int EAPFAST_XMLDataString::svc(DOMNode *n)
{
...
int EAPFAST_XMLDataUInt32::svc(DOMNode *n)
{
   char *c_str = this->getTextContent(n);
   if (c_str) {
...
int EAPFAST_XMLDataBoolean::svc (DOMNode *n)
{
	char *c_str = this->getTextContent(n);
	if (c_str)
	{
