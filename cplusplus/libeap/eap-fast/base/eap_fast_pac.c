...
static int eap_fast_init_pac_data(const char *pac_file,
		      ...
		      const char *pac_file)
{
	...
	if (eap_fast_init_pac_data(pac_file, &rc) < 0)
		return 0;

	if (eap_fast_read_line(&rc, &pos) < 0 ||
	    strcmp(pac_file_hdr, rc.buf) != 0)
	...
	if (err) {
		printf("EAP-FAST: %s in '%s:%d'",
			   err, pac_file, rc.line);
		return -1;
	}

	printf("EAP-FAST: Read %d PAC entries from '%s'",
		   count, pac_file);
...
static int eap_fast_write_pac(const char *pac_file,
			      char *buf, size_t len)
{
	if (strncmp(pac_file, "blob://", 7) == 0) {
		...
		f = fopen(pac_file, "wb");
		if (f == NULL) {
			printf("EAP-FAST: Failed to open PAC "
				   "file '%s' for writing", pac_file);
			return -1;
		}
		if (fwrite(buf, 1, len, f) != len) {
			printf("EAP-FAST: Failed to write all "
				 ...
				 char **pos, size_t *buf_len)
{
	...
	*pos += ret;
	...
	if (*buf == NULL) {
		printf("EAP-FAST: No memory for PAC "
			   "data");
		return -1;
	}
		      ...
		      const char *pac_file)
{
	...
	if (eap_fast_write_pac( pac_file, buf, pos - buf)) {
		free(buf);
		return -1;
	}

	if(MSG_DEBUG)printf("EAP-FAST: Wrote %d PAC entries into '%s'",
		   count, pac_file);
...
static void eap_fast_pac_get_a_id(struct eap_fast_pac *pac)
{
	...
	while (pos + 4 < end) {
		...
		if (type == PAC_TYPE_A_ID) {
			...
			if (pac->a_id == NULL)
				break;
			memcpy(pac->a_id, pos, len);
			pac->a_id_len = len;
		}

		if (type == PAC_TYPE_A_ID_INFO) {
			free(pac->a_id_info);
			pac->a_id_info = malloc(len);
			if (pac->a_id_info == NULL)
				break;
			memcpy(pac->a_id_info, pos, len);
			  ...
			  const char *pac_file)
{
	...
	*pac_root = NULL;
	...
	if (len < 6 || WPA_GET_BE32(buf) != EAP_FAST_PAC_BINARY_MAGIC ||
		...
		printf("EAP-FAST: Invalid PAC file '%s' (bin)",
			   pac_file);
		if (blob == NULL)
			free(buf);
		return -1;
	...
	while (pos < end) {
		...
		pos += 2;
		memcpy(pac->pac_key, pos, EAP_FAST_PAC_KEY_LEN);
		pos += EAP_FAST_PAC_KEY_LEN;
		pac->pac_opaque_len = WPA_GET_BE16(pos);
		pos += 2;
		if (pos + pac->pac_opaque_len + 2 > end)
			goto parse_fail;
		pac->pac_opaque = malloc(pac->pac_opaque_len);
		if (pac->pac_opaque == NULL)
			goto parse_fail;
		memcpy(pac->pac_opaque, pos, pac->pac_opaque_len);
		...
		if (pac->pac_info == NULL)
			goto parse_fail;
		memcpy(pac->pac_info, pos, pac->pac_info_len);
	...
	if(MSG_DEBUG) printf("EAP-FAST: Read %lu PAC entries from '%s' (bin)",
		   (unsigned long) count, pac_file);

	return 0;

parse_fail:
	printf("EAP-FAST: Failed to parse PAC file '%s' (bin)",
		   pac_file);
	if (blob == NULL)
		free(buf);
	if (pac)
			  ...
			  const char *pac_file)
{
	...
	if (eap_fast_write_pac(pac_file, (char *) buf, len)) {
		free(buf);
		return -1;
	}

	if(MSG_DEBUG) printf("EAP-FAST: Wrote %lu PAC entries into '%s' "
...
/*void main()
{
	...
	char pac_file[]="./pac";
	int i=eap_fast_load_pac_bin(&pac_root,pac_file);
	char pac_out_file[]="./pac_out";
int j=eap_fast_save_pac_bin(pac_root, pac_out_file);
