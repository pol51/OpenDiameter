...
class EAP_EXPORTS EapInputIdentityTask:public ACE_Task < ACE_MT_SYNCH > {
 public:
	EapInputIdentityTask() {
		thread = 0;
		stateMachine = 0;
	} void Set(EapPeerSwitchStateMachine * sm) {
		stateMachine = sm;
	}

	~EapInputIdentityTask() {
		if (thread != 0)
			ACE_Thread::kill(thread, 2);
	}
	virtual int open() {
	...
	virtual int svc() {
		...
		do {
			std::auto_ptr < ACE_Method_Request >
	...
	EapFutureStringResult InputIdentity() {
		...
		activationQueue.enqueue
		    (new
		     InputIdentityMethodRequest(stateMachine, futureResult));
		return futureResult;
	}
...
class EAP_EXPORTS EapPeerSwitchStateMachine:public EapSwitchStateMachine,
 ...
 public:
	...
	inline void Notify(AAA_Event ev) {
		try {
			EapStateMachine <
			    EapPeerSwitchStateMachine >::Notify(ev);
		} catch(int i) {
			ACE_UNUSED_ARG(i);
	...
	inline bool & NotificationAllowed() {
	...
	inline EapPeerDecision & Decision() {
		return decision;
	}
	...
	inline EapPeerMethodState & MethodState() {
		return methodState;
	}

	/// External events passed from applications and methods.
	enum event {
		EvRxMsg = (-1),	// Message reception event passed from application via EapSession::Receive().
		EvSgPortEnabled = (-2),	// Port enabled event passed from application.
		EvSgValidReq = (-3),	// Integrity check sucess event passed from methods.
		EvSgInvalidReq = (-4),	// Integrity check failure event passed from methods.
		EvSgTunnelEstablished = (-5),
	};
