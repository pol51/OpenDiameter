...
class EapBackendAuthSwitchAction:public AAA_Action <
...
class EapBackendAuthSwitchStateTable_S:public AAA_StateTable <
    EapBackendAuthSwitchStateMachine > {
 ...
 private:
	 class AcDisable:public EapBackendAuthSwitchAction {
		void operator() (EapBackendAuthSwitchStateMachine & sm) {
			EAP_LOG(LM_DEBUG,
				"[eapbackendAuthSwitchstatetable_s] AcDisable\n");
	...
	class AcPickUpMethod:public EapBackendAuthSwitchAction {
		void operator() (EapBackendAuthSwitchStateMachine & sm) {
			...
			if (sm.Policy().SupportPickUp(type)) {
				...
				sm.CreateMethodStateMachine(type,
							    Authenticator,innerType);
	...
	class AcMethodResponse:public EapBackendAuthSwitchAction {
		void operator() (EapBackendAuthSwitchStateMachine & sm) {
			...
			if (msm.IsDone()) {
				...
				sm.MethodState() =
				    EapAuthSwitchStateMachine::END;
				sm.Event(EapAuthSwitchStateMachine::
					 EvSgEndMethod);
			} else {
				sm.MethodState() =
				    EapAuthSwitchStateMachine::CONT;
				sm.Event(EvElse);
			}
		}
	};
	...
	class AcBuildRequest:public EapBackendAuthSwitchAction {
		void operator() (EapBackendAuthSwitchStateMachine & sm) {
			...
			ACE_Byte id = sm.CurrentIdentifier() =
			    sm.MethodStateMachine().GetNextIdentifier(sm.
								      CurrentIdentifier
								      ());
	...
	class AcSendRequest:public EapBackendAuthSwitchAction {
		void operator() (EapBackendAuthSwitchStateMachine & sm) {
			...
			if (sm.RetransmissionEnabled())
				 sm.ScheduleTimer(EvTo,
						  sm.RetransmissionInterval());
	...
	class AcProposeMethod:public EapBackendAuthSwitchAction {
		void operator() (EapBackendAuthSwitchStateMachine & sm) {
			...
			if (!type.IsVSE())
				 ...
				 EAP_LOG(LM_DEBUG,
					 "Backend: Trying another extended method.\n");
			...
			if (type == EapType(1) || type == EapType(2))	// Identity or Notification
				sm.MethodState() =
				 ...
				 sm.MethodState() =
				    EapAuthSwitchStateMachine::PROPOSED;
	...
	class AcDoPolicyCheck:public EapBackendAuthSwitchAction {
		void operator() (EapBackendAuthSwitchStateMachine & sm) {
			EapAuthSwitchStateMachine::EapAuthDecision decision =
			    sm.Decision();
			if (decision ==
			    EapAuthSwitchStateMachine::DecisionSuccess)
				sm.Event(EvSgPolicySat);
			else if (decision ==
				 EapAuthSwitchStateMachine::DecisionFailure)
	...
	class AcDiscard2:public EapBackendAuthSwitchAction {
		void operator() (EapBackendAuthSwitchStateMachine & sm) {
			EAP_LOG(LM_DEBUG,
				"Backend: Message discarded without parsing.\n");
			sm.DiscardCount()++;
			AAAMessageBlock *msg;
	...
	class AcRetransmit:public EapBackendAuthSwitchAction {
		void operator() (EapBackendAuthSwitchStateMachine & sm) {
			if (sm.RetransmissionCount() <
			    sm.MaxRetransmissionCount()) {
				EAP_LOG(LM_DEBUG,
					"Backend: Retransmitting Request.\n");
				...
				EAP_LOG(LM_DEBUG,
					"Backend: Reach max retransmission count.\n");
				sm.CancelTimer();
	...
	class AcWaitTunnelProcess:public EapBackendAuthSwitchAction {
		void operator() (EapBackendAuthSwitchStateMachine & sm) {
			EAP_LOG(LM_DEBUG,
				"EapStandAloneAuthSwitchAction: AcWaitTunnelProcess.\n");
	...
	class AcTunnelEstablished:public EapBackendAuthSwitchAction {
		void operator() (EapBackendAuthSwitchStateMachine & sm) {
	...
	class AcNotifyInnerEap:public EapBackendAuthSwitchAction {
		void operator() (EapBackendAuthSwitchStateMachine & sm) {
			sm.InnerMethodStateMachine().Notify
			    (EapMethodStateMachine::EvSgIntegrityCheck);
	...
	enum state {
		...
		StTunnelEstablished,
		StWaitTunnelProcess,
		StTunnelMethodRequest,
	};

	EapBackendAuthSwitchStateTable_S() {

		AddStateTableEntry(StBegin,
				   EapAuthSwitchStateMachine::EvSgRestart,
		...
		AddStateTableEntry(StInitialize, EvRxNonResp, StSelectAction,
				   acDoPolicyCheck);
		AddStateTableEntry(StInitialize, EvElse, StPickUpMethod,
				   acPickUpMethod);
		...
		AddStateTableEntry(StInitialize, EvRxNak, StNak, acResetMethod);
		...
		AddStateTableEntry(StIntegrityCheck,
				   EapAuthSwitchStateMachine::EvRxMsg,
		...
		AddStateTableEntry(StRetransmit, EvSgMaxRetransmission,
				   StTimeoutFailure);
		AddStateTableEntry(StRetransmit, EvElse, StIdle);

		AddStateTableEntry(StNak, EvUCT, StSelectAction,
				   acDoPolicyCheck);

		AddStateTableEntry(StDiscard, EvUCT, StIdle);

		AddStateTableEntry(StSuccess,
				   EapAuthSwitchStateMachine::EvRxMsg,
				   StSuccess, acDiscard2);
		AddWildcardStateTableEntry(StSuccess, StSuccess);
		AddStateTableEntry(StFailure,
				   EapAuthSwitchStateMachine::EvRxMsg,
		...
		AddStateTableEntry(StIntegrityCheck,
				   ...
				   EvSgTunnelEstablished, StTunnelEstablished,
				   acTunnelEstablished);
...
typedef ACE_Singleton < EapBackendAuthSwitchStateTable_S,
    ACE_Recursive_Thread_Mutex > EapBackendAuthSwitchStateTable;

 EapBackendAuthSwitchStateMachine::EapBackendAuthSwitchStateMachine(ACE_Reactor & r, EapJobHandle & h):
EapAuthSwitchStateMachine(r, h),
    EapStateMachine < EapBackendAuthSwitchStateMachine >
    (*this, *EapBackendAuthSwitchStateTable::instance(), r, *this,
     (char *)"backend") {
