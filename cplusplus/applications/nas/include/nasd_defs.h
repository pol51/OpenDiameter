...
class NASD_Map {
 ...
 public:
	 virtual ~ NASD_Map() {
		...
		if (i == m_Map.end()) {
			...
			m_Map.insert(std::pair < std::string,
				     NASD_MapElement * >(e->Name(), item));
			e.release();
			return true;
		}
		return false;
 ...
 template < class PROTOCOL > class NASD_ProtocolElement:
...
typedef NASD_ProtocolElement < NASD_ApPanaElement > NASD_ApPanaData;
...
typedef NASD_ProtocolElement < NASD_AaaLocalEapAuth > NASD_AaaLocalEapAuthData;
typedef NASD_ProtocolElement<NASD_AaaDiameterEap> NASD_AaaDiameterEapData;

typedef NASD_Map NASD_AaaProtocolTable;

 template < class POLICY > class NASD_PolicyElement:
public NASD_MapElement {
 public:
	POLICY & Policy() {
		return m_Policy;
...
typedef NASD_PolicyElement < NASD_ScriptPolicy > NASD_PolicyScriptData;
typedef NASD_Map NASD_PolicyTable;

class NASD_RouteEntry: public NASD_MapElement {
 public:
 ...
 public:
	...
	virtual void Dump() {
		...
		for (i = m_PolicyList.begin(); i != m_PolicyList.end(); i++) {
			NASD_LOG(LM_INFO,
				 "(%P|%t)         Policy: %s\n", (*i).data());
		}
	}
...
class NASD_RouteMap:public NASD_Map {
 public:
...
class NASD_CallManagementData {
 public:
	...
	void Dump() {
		NASD_LOG(LM_INFO, "(%P|%t)--- Call Management ---\n");
		NASD_LOG(LM_INFO,
			 "(%P|%t)     Thread Cnt: %d\n", m_ThreadCount);

		NASD_LOG(LM_INFO, "(%P|%t)--- Call Routing    ---\n");
		m_CallRouteTable.Dump();

		NASD_LOG(LM_INFO, "(%P|%t)--- Access Protocol ---\n");
		m_AccessProtocolTable.Dump();

		NASD_LOG(LM_INFO, "(%P|%t)---   AAA Protocol  ---\n");
		m_AAAProtocolTable.Dump();

		NASD_LOG(LM_INFO, "(%P|%t)---  Access Policy  ---\n");
		m_PolicyTable.Dump();
	}
 protected:
	int m_ThreadCount;
...
typedef ACE_Singleton < NASD_CallManagementData,
    ACE_Recursive_Thread_Mutex > NASD_CallManagementData_S;
