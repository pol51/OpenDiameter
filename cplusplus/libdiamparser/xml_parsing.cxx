  ...
  public AAAXML_Element
{
  public:
     ...
     virtual bool startElement (ACEXML_Attributes *alist) {
        ...
        if (alist != 0) {
            for (size_t i = 0; i < alist->getLength (); ++i) {
               if (! ACE_OS::strcmp(alist->getQName(i), "protocol")) {
                   ACE_DEBUG ((LM_INFO,
                               ACE_TEXT (" Dictionary Protocol = \"%s\"\n"),
                               alist->getValue (i)));
                   m_protocol = ACE_OS::atoi(alist->getValue(i));
                   break;
               }
            }
        }
     ...
     static int Protocol(AAAXML_ElementStack &stack) {
        if (stack.empty()) {
            ACE_DEBUG((LM_ERROR, "Call stack is empty !!!\n"));
            throw;
        }
        AAAXML_DictionaryElement *dict = 
            (AAAXML_DictionaryElement*)stack.back();
        if (dict->Name() != std::string("dictionary")) {
            ACE_DEBUG((LM_ERROR, 
              "Call stack root is not a dictionary !!!\n"));
            throw;
        }
  ...
  public AAAXML_Element
{
  public:
     ...
     virtual bool startElement(ACEXML_Attributes *alist) {
        ...
        m_command = new AAACommand;
        m_command->avp_f = new AAAQualifiedAvpList(PARSE_TYPE_FIXED_HEAD);
        m_command->avp_r = new AAAQualifiedAvpList(PARSE_TYPE_REQUIRED);
        m_command->avp_o = new AAAQualifiedAvpList(PARSE_TYPE_OPTIONAL);
        m_command->avp_f2 = new AAAQualifiedAvpList(PARSE_TYPE_FIXED_TAIL);

        m_command->protocol = AAAXML_DictionaryElement::Protocol(CallStack());
        ...
        m_command->flags.r = 1;
        AAACommandList::instance()->add(m_command);
     ...
     virtual bool startElement (ACEXML_Attributes *alist) {
        ...
        if (alist != 0) {
            ...
            m_avp = new AAADictionaryEntry(code,
			                   ...
			                   vendorId,
			                   AAA_AVP_FLAG_NONE,
                                           AAAXML_DictionaryElement::Protocol(CallStack()));
            m_avp->flags = AAA_AVP_FLAG_NONE;
            m_avp->flags |= 
                ((encrypt == std::string("yes")) ? AAA_AVP_FLAG_ENCRYPT : 0);
            m_avp->flags |= 
            ...
            m_avp->flags |= 
                ((vendorId == 0) ? 0 : AAA_AVP_FLAG_VENDOR_SPECIFIC);
  ...
  public AAAXML_Element
{
  public:
     ...
     virtual bool startElement (ACEXML_Attributes *alist) {
        ...
        AvpType *avpt = AvpTypeList::instance()->search(tname.c_str());
        if (avpt == NULL) {
            ACE_DEBUG((LM_ERROR, 
                 "Unknown AVP type %s.\n", tname.c_str()));
            throw;
  ...
  public AAAXML_Element
{
  public:
     ...
     virtual bool startElement (ACEXML_Attributes *alist) {
        ...
        AAAXML_AvpElement *avpElm = (AAAXML_AvpElement*)Parent();
        m_grpAvp = new AAAGroupedAVP;
        m_grpAvp->code = avpElm->Avp()->avpCode;
        m_grpAvp->vendorId = avpElm->Avp()->vendorId;
        m_grpAvp->avp_f = new AAAQualifiedAvpList(PARSE_TYPE_FIXED_HEAD);
        m_grpAvp->avp_r = new AAAQualifiedAvpList(PARSE_TYPE_REQUIRED);
        m_grpAvp->avp_o = new AAAQualifiedAvpList(PARSE_TYPE_OPTIONAL);
        m_grpAvp->avp_f2 = new AAAQualifiedAvpList(PARSE_TYPE_FIXED_TAIL);
  ...
  public AAAXML_Element
{
  public:
     ...
     AAAQualifiedAvpList *ResolveAvpList(AAADictionary *dict, 
                                         std::string &position) {
        if (dict == NULL) {
  ...
  public AAAXML_Element
{
  public:
     ...
     virtual bool startElement (ACEXML_Attributes *alist) {
        ...
        if ((avp = AAAAvpList::instance()->search(avpName,
            AAAXML_DictionaryElement::Protocol(CallStack()))) == NULL) {
